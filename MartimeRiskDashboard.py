...
Outputs to: index.html and Data/Maritime/Html/maritime_dashboard_YYYY-MM-DD_HHMMSS.html
"""
...
class MaritimeDashboard:
    ...
    def generate_html(self) -> tuple:
        """
        Generate HTML dashboard to two locations
        
        Returns:
            tuple: (index_path, archived_path)
        """
...
<body>
    <div class="container">
        ...
        # Output 1: index.html (root directory - main/latest version)
        index_path = "index.html"
        with open(index_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        print(f"✓ Dashboard saved to: {index_path}")
        
        # Output 2: Data/Maritime/Html/ with timestamp (archive)
        self._ensure_directory_exists("Data/Maritime/Html")
        archived_filename = f"maritime_dashboard_{self.file_timestamp}.html"
        archived_path = os.path.join("Data/Maritime/Html", archived_filename)
        with open(archived_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        print(f"✓ Dashboard archived to: {archived_path}")
        
        return (index_path, archived_path)
...
def main():
    ...
    # Generate HTML to both locations
    index_file, archived_file = dashboard.generate_html()
    
    print(f"\n✓ Dashboard generation complete!")
    print(f"   Main: {index_file}")
